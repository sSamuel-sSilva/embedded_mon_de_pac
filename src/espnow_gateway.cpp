...
static const int ESP_PING_TIMEOUT = 6000;
...
general_status espnow_check_ping()
{
    ...
    for(int i = 0; i < MAX_PEER_CON; i++)
    {
        if ((emmiters[i].code != 0) && (millis() - emmiters[i].last_ping > ESP_PING_TIMEOUT))
        {
            emmiters[i].status = ESPNOW_DISCONNECTED;
            peers_fault++;
        } else if ((emmiters[i].code != 0) && (millis() - emmiters[i].last_ping < ESP_PING_TIMEOUT))
        {
            emmiters[i].status = ESPNOW_CONNECTED;
        }
    }
